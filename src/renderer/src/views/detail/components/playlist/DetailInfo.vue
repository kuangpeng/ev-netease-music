<template>
  <n-space type="flex" justify="start" :size="[20, 0]">
    <div class="detail-cover b-rd-1 overflow-hidden text-0">
      <n-image width="184" :src="detail?.coverImgUrl" :fallback-src="defaultCover" />
    </div>
    <div class="detail-info">
      ...
      <div class="info-author mt-1">
        <n-space type="flex" align="center" :wrap-item="false" :size="[8, 0]">
          <n-avatar round :size="24" :src="detail?.creator?.avatarUrl">
            <template #fallback>
              <n-icon size="30" :component="MdContact" />
            </template>
          </n-avatar>
          <router-link class="link text-1" :to="'/user/' + detail?.creator?.userId || '0'">{{
      ...
      <div class="mt-1 color-gray-600">
        歌曲 : <span class="color-gray-500">{{ detail?.trackCount }}</span
        >&nbsp;&nbsp;&nbsp;&nbsp;播放 : <span class="color-gray-500">{{ text_playcount }}</span>
      </div>
      <div class="mt-1 color-gray-600">
